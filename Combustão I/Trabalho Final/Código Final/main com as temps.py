...
# %%
#FUNÇÃO DELTA T PARA UM DADO PHI

def delta_T_phi(array, phi):
    xs = [x[0] for x in array]
    ys = [x[1] for x in array]
    ...
    degree = 15 #melhor aproximação encontrada
    coefs, res, _, _, _ = np.polyfit(xs,ys,degree, full = True)
    ffit = np.poly1d(coefs)
...
# %%
#FUNÇÃO TEMPERATURA POR ZONA

def temperaturas(A_ft, D_ft, L_ZP, L_ZS, L_ZD, L_ZR, L_CC, T3, P3, m_dot_comb, array, phi_pobre, phi_rico, phi_ZP, phi_ZS, phi_global):

  p3 = (1- 0.06)*P3

  #Zona de recirculação

  deltaT_ZR = delta_T_phi(array,1)
  eta_ZR = 0.56 + 0.44*np.tanh((1.5475*10**-3)*(T3 + 108*np.log(p3) - 1863))
  T_out_ZR = T3 + eta_ZR*deltaT_ZR
  T_mean_ZR = (1/3)*T3 + (2/3)*T_out_ZR
  T_out_ZR = T_mean_ZR


  #Zona primária

  deltaT_ZP = delta_T_phi(array,phi_ZP)
  eta_ZP = 0.71 + 0.29*np.tanh((1.5475*10**-3)*(T3 + 108*np.log(p3) - 1863))
  T_out_ZP = T3 + eta_ZP*deltaT_ZP


  #Zona secundária

  P3_sobre_delta_P = 1/0.06
  D_estrela = 0.736 - 0.0173*(P3_sobre_delta_P)

  #misturas pobres

  if phi_ZS < 0.5:
    n = 1
  elif phi_ZS > 0.5 and phi_ZS <= 1:
    n = 2*phi_ZS
  elif phi_ZS > 1 and phi_ZS <= 2:
    n = 2/phi_ZS
  elif phi_ZS > 2:
    n = 1

  V_ZS = A_ft*0.5*D_ft
  psi_T3 = (m_dot_comb/(V_ZS*P3**n))/(10**(-3.054*phi_ZS**(-1.205))*T3**(1.2327*phi_ZS**(-1.205)))
  #psi_T3 = 10**(-3.054*phi_ZS**(-1.205))*T3**(1.2327*phi_ZS**(-1.205))*(m_dot_comb/(V_ZS*P3**n))
  eta_ZS_pobre = 1/(10**(10**(0.911*np.log10(psi_T3) + 8.02*phi_ZS - 1.097 + D_estrela)))
  
  #misturas ricas

  #eta_ZS_rico = 1/phi_ZS


  T_out_ZS_pobre = T3 + eta_ZS_pobre*delta_T_phi(array,phi_ZS)
  #T_out_ZS_rico = T3 + eta_ZS_rico*delta_T_phi(array,phi_rico)
  T_out_ZS = T_out_ZS_pobre
  ...
  if phi_ZD < 0.5:
    n = 1
  elif phi_ZD > 0.5 and phi_ZD <= 1:
    n = 2*phi_ZD
  elif phi_ZD > 1 and phi_ZD <= 2:
    n = 2/phi_ZD
  elif phi_ZD > 2:
    n = 1

  V_ZD = A_ft*1.5*D_ft
  psi_T3 = (m_dot_comb/(V_ZD*P3**n))/(10**(-3.054*phi_ZD**(-1.205))*T3**(1.2327*phi_ZD**(-1.205)))
  #psi_T3 = 10**(-3.054*phi_ZD**(-1.205))*T3**(1.2327*phi_ZD**(-1.205))*(m_dot_comb/(V_ZD*P3**n))
  eta_ZD_pobre = 1/(10**(10**(0.911*np.log10(psi_T3) + 8.02*phi_ZD - 1.097 + D_estrela)))

  #misturas ricas
  #eta_ZD_rico = 1/phi_ZD



  T_out_ZD_pobre = T3 + eta_ZD_pobre*delta_T_phi(array,phi_ZD)
  #T_out_ZD_rico = T3 + eta_ZD_rico*delta_T_phi(array,phi_rico)
  T_out_ZD = T_out_ZD_pobre

  #plota o gráfico de temperatura por distância

  comp1 = np.linspace(0, L_ZR)
  comp2 = np.linspace(L_ZR, L_ZP)
  comp3 = np.linspace(L_ZP, (L_ZP + L_ZS))
  comp4 = np.linspace((L_ZP+L_ZS), L_CC)

  def temp_ZR(x):
    return (x/x)*T_out_ZR

  def temp_ZP(x):
    return T_out_ZR + ((T_out_ZP - T_out_ZR)/(L_ZP - L_ZR)*(x-L_ZR))

  def temp_ZS(x):
    return T_out_ZP + ((T_out_ZS - T_out_ZP)/L_ZS)*(x-L_ZP)

  def temp_ZD(x):
    return T_out_ZS + ((T_out_ZD - T_out_ZS)/L_ZD)*(x - L_ZP - L_ZS)
  ...
  for i in comp1:
    temp_1.append(temp_ZR(i))

  for i in comp2:
    temp_2.append(temp_ZP(i))

  for i in comp3:
    temp_3.append(temp_ZS(i))

  for i in comp4:
    temp_4.append(temp_ZD(i))

  plt.figure(figsize = (15,10))
  plt.plot(comp1*1000, temp_1, label='Zona de Recirc.')
  plt.plot(comp2*1000, temp_2, label='Zona Primária')
  plt.plot(comp3*1000, temp_3, label='Zona Secundária')
  plt.plot(comp4*1000, temp_4, label='Zona de Diluição')
  ...
  return T_out_ZR, T_out_ZP, T_out_ZS, T_out_ZD
...
temps_1 = temperaturas(A_ft_1, D_ft_1, L_ZP_1, L_ZS_1, L_ZD_1, L_ZR_1, L_CC_1, T3_1, P3_1, m_comb_1, array_1, phi_pobre_1, phi_rico_1, phi_ZP_1, phi_ZS_1, phi_global_1)
...
temps_2 = temperaturas(A_ft_2, D_ft_2, L_ZP_2, L_ZS_2, L_ZD_2, L_ZR_2, L_CC_2, T3_2, P3_2, m_comb_2, array_2, phi_pobre_2, phi_rico_2, phi_ZP_2, phi_ZS_2, phi_global_2)
...
temps_3 = temperaturas(A_ft_3, D_ft_3, L_ZP_3, L_ZS_3, L_ZD_3, L_ZR_3, L_CC_3, T3_3, P3_3, m_comb_3, array_3, phi_pobre_3, phi_rico_3, phi_ZP_3, phi_ZS_3, phi_global_3)
...
temps_4 = temperaturas(A_ft_4, D_ft_4, L_ZP_4, L_ZS_4, L_ZD_4, L_ZR_4, L_CC_4, T3_4, P3_4, m_comb_4, array_4, phi_pobre_4, phi_rico_4, phi_ZP_4, phi_ZS_4, phi_global_4)
